...
def train(ds_mhc, ds_peps, ds_ics):
	
	# 训练数据对总数
	num_samples = len(ds_mhc)
	## 输入和标签
	input_mhc = tf.placeholder(tf.int32, [BATCH_SIZE, None]) # 01 mat
	input_peps = tf.placeholder(mainInf.tf_float_, [BATCH_SIZE, NUM_PEPGROUP, PEP_HEIGHT*PEP_WIDTH])
	input_ics = tf.placeholder(mainInf.tf_float_, [BATCH_SIZE, NUM_PEPGROUP])
	## 参数正则化
...
#=================================================================================================================
def main(argv=None):
	input_mhc = loadFile.load_data(SRC_FILE_EVAL_MHC)
	input_peps = loadFile.load_data(SRC_FILE_EVAL_PEP)
	input_ics = loadFile.load_data(SRC_FILE_EVAL_IC)
